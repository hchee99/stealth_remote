...
import json
import os
import keyboard  # 🌟 새로 설치한 백그라운드 키보드 라이브러리
from PyQt5.QtCore import QUrl, Qt, pyqtSignal
from PyQt5.QtGui import QKeySequence
from PyQt5.QtWidgets import (QApplication, QMainWindow, QWidget, QVBoxLayout, QHBoxLayout, 
                             QSlider, QLabel, QPushButton, QDialog, QLineEdit, QFormLayout, QShortcut)
from PyQt5.QtWebEngineWidgets import QWebEngineView

# 🗂️ 설정 파일 이름
CONFIG_FILE = 'stealth_config.json'

def load_config():
    if os.path.exists(CONFIG_FILE):
        with open(CONFIG_FILE, 'r') as f:
            return json.load(f)
    return {"opacity": 30, "panic_key": "F12", "hide_key": "C", "exit_key": "Esc"}
...
class SettingsDialog(QDialog):
    def __init__(self, current_config, parent=None):
        ...
        self.setWindowTitle("⚙️ 은폐 환경 설정")
...
class UltimateStealthPlayer(QMainWindow):
    ...
    def __init__(self, config):
        ...
        self.drag_handle.setFixedHeight(25) # 🌟 이렇게 글자를 변경해주세요!
    ...
    #         self.drag_handle.setFixedSize(self.width(), 25)
    ...
    def closeEvent(self, event):
        keyboard.unhook_all() # 🌟 프로그램 종료 시 키보드 감시도 깔끔하게 해제
        self.remote.close()
